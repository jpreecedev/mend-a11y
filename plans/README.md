...
| [012](012-message-channel-hardening.md) | Validate senders and constrain the dashboard-key relay | P1 | M | — | DONE 2026-08-06 — merged (executor `7b5c6ef`+`59a3a67`). `guardSender()` runs before every branch; relay accepts only the configured dashboard origin and no longer rewrites `dashboardUrl`. **Plan's platform assumption corrected by evidence**: Chrome populates `sender.tab` for side-panel pages, so the panel boundary is `sender.id` + browser-set `chrome-extension://` `sender.url` (unforgeable from content scripts), not tab-absence. 20 checks in `test/message-guard.test.ts` + fixtures updated in three suites (incl. one cross-plan integration fix to highlight-clear's senders caught at merge review). Deferred, recorded in the plan: nonce handshake (needs mend-website half), confirm-before-replace of a non-empty key, hasKey-instead-of-raw-key |
...
| [014](014-sync-retry-recovery.md) | Stop a doomed first upload from permanently blocking an audit's save | P1 | S | — | DONE 2026-08-06 — merged (executor `21468f2`). `TextField` gains opt-in `commitOn="blur"` (local draft + committed-ref so relay-delivered keys resync without stomping typing); key field uses it; `syncState.ts#clearErrorEntries` drops error entries when the key string changes, re-arming the auto-upload guard. 7 checks in `test/sync-rearm.test.ts`. Untested by automation (flagged by executor): the blur-vs-input distinction itself (needs browser-level test) and the full type→401→correct→saved walk |
| [015](015-audit-lifecycle-tests.md) | Characterization tests around the audit lifecycle | P1 | M | — | DONE 2026-08-06 — merged (executor `test/audit.test.ts`, 17 checks). Injection ladder, error classification (restricted vs missing-grant vs generic), timeout via a >=45s-only setTimeout patch restored in finally, empty-url re-read, cache write, and the two plan-016 tripwires (result:null ⇒ partial pinned true; result:undefined ⇒ partial pinned false). Zero src/ diff |
| [016](016-partial-audit-honesty.md) | Surface partial audits; make partial detection real | P2 | S | 015 | DONE 2026-08-06 — merged (executor `1f58bd8`). Live-probed Chrome 148: a script-blocked frame reports `result: null` (the original check was real, not dead); `error` never appears on `chrome.scripting.InjectionResult` (only the unrelated userScripts type has it); successful file injections complete with `true`. Predicate now flags error/null/undefined on non-top frames (false-partial beats false-complete, reasoning recorded inline); `PartialBanner` (role=status) on Results + Pass; audit suite now 19 checks. Accepted blind spot documented: a frame omitted from the results array entirely |
...
| [018](018-panel-self-audit.md) | Audit the auditor: engine run against the panel in smoke | P2 | S | — | DONE 2026-08-06 — merged (executor `3d5d59d`+`855738a`). **The self-audit found real violations and fixed them**: no `<main>` landmark (all panel content sat outside landmarks); `.kbd-hint` 2.93:1 and `.link-btn` 4.36:1 in light; `.kbd-hint` 4.38:1 and `.btn.primary` **2.88:1** in dark — fixed via token adjustments + a new `--ap-on-accent` (dark ink on accent in dark theme), each with the measured ratio recorded in panel.css comments. Smoke is now 7 checks: engine injected via CDP evaluate (extension-page CSP silently drops addScriptTag), theme-settle waits + transition-freeze make the measurement deterministic, console errors now fail the run. SUBMISSION.md's 'never been measured' box is now obsolete (file deliberately untouched — mirrors store console) |
| [019](019-local-export.md) | Local JSON export for keyless users | P3 | S–M | — (sequenced after 016) | DONE 2026-08-06 — merged (executor `07072d1`, one STOP round). Executor's 320px probe caught the head-actions row overflowing with a 4th button (338px in 282px, Save clipped) and STOPPED per plan; reviewer decision: `flex-wrap: wrap` on `.head-actions` (re-measured clean in both button states, Save wraps to its own line). `export.ts` reuses `buildIngestPayload` with byte-parity pinned in `test/export.test.ts` (6 checks); DownloadIcon added; buttons unconditional; no new permissions (`downloads` absent from built manifest). Known gap, deliberate: the smoke self-audit covers only the empty state, so the new buttons aren't axe-audited (018's scoping) |
